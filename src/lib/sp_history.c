...
void sp_history(lv_obj_t *parent) {
    ...
    for(int i = 0; i < history_list->count; i++) {
        ...
        char *title = history_list->list[i].title;

        lv_obj_t *label = lv_label_create(cont);
        lv_label_set_text_fmt(label, "%s  %s", i == 0 ? "（最新）" : "", title);
        lv_obj_set_size(label, LV_PCT(100), LV_SIZE_CONTENT);
        lv_obj_set_style_text_font(label, &lv_font_han_sans_20_3500, 0);

        lv_obj_t *content_page = lv_menu_page_create(menu, NULL);

        lv_obj_t *content_title = lv_label_create(content_page);
        lv_obj_set_size(content_title, LV_PCT(100), LV_SIZE_CONTENT);
        lv_label_set_text_fmt(content_title, "%s\n", title);
        lv_obj_set_style_text_font(content_title, &lv_font_han_sans_20_3500, 0);
        lv_obj_set_style_text_align(content_title, LV_TEXT_ALIGN_CENTER, 0);
